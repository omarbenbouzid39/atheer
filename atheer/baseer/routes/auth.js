...
router.post('/register', async (req, res) => {
  try {
    ...
    // توليد handle — إما مخصص أو تلقائي
    let handle;
    const customUsername = (req.body.username || '').trim();
    if (customUsername) {
      if (!/^[a-zA-Z0-9_]{3,20}$/.test(customUsername))
        return res.status(400).json({ error: 'اسم المستخدم: 3-20 حرف (أرقام، حروف، _)' });
      handle = '@' + customUsername;
      const taken = await User.findOne({ handle });
      if (taken) return res.status(409).json({ error: 'اسم المستخدم مأخوذ، جرّب آخر' });
    } else {
      handle = '@' + name.trim().toLowerCase().replace(/\s+/g, '.').replace(/[^a-z0-9.]/g, '') || '@user';
