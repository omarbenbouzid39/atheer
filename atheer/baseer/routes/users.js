...
router.get('/:handle/following', authOptional, async (req, res) => {
  try {
    const handle = req.params.handle.startsWith('@') ? req.params.handle : '@' + req.params.handle;
    const owner  = await User.findOne({ handle }).lean();
    if (!owner) return res.status(404).json({ error: 'غير موجود' });
    const follows = await Follow.find({ follower_id: owner._id })
      .populate('following_id', 'name handle avatar_url verified')
      .lean();
    res.json(follows.map(f => ({
      id:        f.following_id?._id,
      name:      f.following_id?.name,
      handle:    f.following_id?.handle,
      avatar_url:f.following_id?.avatar_url,
      verified:  f.following_id?.verified,
    })));
  } catch(e) { res.status(500).json({ error: 'خطأ في الخادم' }); }
});

// ── GET /api/users/:handle/followers ─────────────────────────────────────────
router.get('/:handle/followers', authOptional, async (req, res) => {
  try {
    const handle = req.params.handle.startsWith('@') ? req.params.handle : '@' + req.params.handle;
    const owner  = await User.findOne({ handle }).lean();
    if (!owner) return res.status(404).json({ error: 'غير موجود' });
    const follows = await Follow.find({ following_id: owner._id })
      .populate('follower_id', 'name handle avatar_url verified')
      .lean();
    res.json(follows.map(f => ({
      id:        f.follower_id?._id,
      name:      f.follower_id?.name,
      handle:    f.follower_id?.handle,
      avatar_url:f.follower_id?.avatar_url,
      verified:  f.follower_id?.verified,
    })));
  } catch(e) { res.status(500).json({ error: 'خطأ في الخادم' }); }
});
